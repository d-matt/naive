...
<div id="content" class="two_col">
  ...
  <form class="filter" method="post" id="search_form" name="search" action="{$F_SEARCH_ACTION}">
    ...
    <fieldset>
      ...
      {$TAG_SELECTION}
      <label>
        <span><input type="radio" name="tag_mode" value="AND" checked="checked"> {'All tags'|@translate}</span>
      </label>
      <label>
        <span><input type="radio" name="tag_mode" value="OR"> {'Any tag'|@translate}</span>
      </label>
    </fieldset>
    {/if}
